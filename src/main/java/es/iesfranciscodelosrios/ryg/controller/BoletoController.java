...
public class BoletoController {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "OperaciÃ³n exitosa", response = List.class),
			@ApiResponse(code = 404, message = "Entregado no vÃ¡lido/no hay boletos entregados"),
			@ApiResponse(code = 500, message = "Internal server error") })
	@CrossOrigin(origins = "http://localhost:8100")
	...
	/**
	 * Método que recoge una petición http a nuestra API para hacer una consulta a
	 * la base de datos y aÃ±adir un boleto.
	 * 
	...
	/**
	 * Método que obtiene un boleto de manera aleatoria, que no esté entregado y distinto del 
	 * usuario que le pasamos con parámetro.
	 * 
